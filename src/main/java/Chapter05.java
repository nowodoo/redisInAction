...
public class Chapter05 {
    ...
    public Map<String, Object> getConfig(Jedis conn, String type, String component) {
        ...
        if (lastChecked == null || lastChecked < System.currentTimeMillis() - wait) {
            ...
            if (value != null) {
                Gson gson = new Gson();
                config = (Map<String, Object>) gson.fromJson(value, new TypeToken<Map<String, Object>>() {}.getType());
            } else {
        ...
        try {
            ...
            while ((line = parser.getLine()) != null) {
                ...
                if (startIp.indexOf('.') != -1) {
                    score = ipToScore(startIp);  //将ip转换为分数
                } else {
                    try {
                ...
                conn.zadd("ip2cityid:", score, cityId);  //只要是存在一个ip,那么就将这个ip的分值加1
                count++;
            }
        } catch (Exception e) {
    ...
    public class AccessTimer {
        ...
        public void stop(String context) {
            ...
            Transaction trans = conn.multi();
            trans.zadd("slowest:AccessTime", average, context);  //将时间耗费添加进去
            trans.zremrangeByRank("slowest:AccessTime", 0, -101);  //只是保留前100个就好了
            trans.exec();
        }
    }
}
